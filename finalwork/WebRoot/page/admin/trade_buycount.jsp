...
<html>
  <head>
    ...
    <title>My JSP 'trade_buycount.jsp' starting page</title>
    		...
    		<sql:setDataSource var="snapshot" driver="com.mysql.jdbc.Driver"
     		...
     		<c:choose>
     			<c:when test="${empty param.tname and empty param.tid}">
     				<c:set var="mysql" value="select * from trade order by buy_count desc limit 0,10"></c:set>
     			</c:when>
     			<c:otherwise>
     				<c:if test="${not empty param.tname}">
     					<c:set var="mysql" value="select * from trade where tname like '%${param.tname}%'order by buy_count desc limit 0,10"></c:set>
     				</c:if>
     		...
     		<tr>
     			<td>商品id</td><td>商品名</td><td>商品购买量</td><td></td>
     		</tr>
